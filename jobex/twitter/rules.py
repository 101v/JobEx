...
import requests
from jobex.twitter import bearer_token
from jobex.twitter import urls
...
def commit_filter_rules_if_required():
    ...
    effective_rules = get_rule_list_from_file("jobex/twitter/rules.txt")
...
def get_rule_list_from_file(file_name):
    ...
    with open(file_name) as f:
        ...
        rule_list = [rule.strip() for rule in rule_list if rule is not None and len(rule.strip()) > 0 and not rule.strip().startswith("#")]
    return rule_list


def get_rule_list_from_twitter_account():
    with requests.get(
        urls.rule_commit_endpoint,
